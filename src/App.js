...
const MINIMO_QTD={"V":5,"VDup":3,"Val/GL":3,"Val/Lin":2,"VaL":3,"VX":5,"VL":5,"VD":5,"VXC":5,"V/FiC":5,"VXP":5,"TL":5,"TX":5,"TP":5,"LDR":5,"LDRD":3,"CXPI":1};
...
const ProductModal=memo(({product:p,cartCount,onClose,onAdd})=>{
  ...
  const[qt,setQt]=useState(MINIMO_QTD[p.sku]||1);
  const[er,setEr]=useState("");
  const min=MINIMO_QTD[p.sku]||1;
  const cor=p.cores[cl]||p.cores[0];
  const tam=p.sizes[sz]||p.sizes[0];

  const doAdd=()=>{
    if(qt<min){setEr(`Mínimo de ${min} unidades.`);return;}
    setEr("");
      ...
      <div className="scr" style={{background:BG,borderRadius:"24px 24px 0 0",width:"100%",maxWidth:420,maxHeight:"92vh",borderTop:`2px solid ${VERDE}`}}
        ...
        <div style={{padding:"16px 16px 40px"}}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
            {p.sizes.map((s,i)=>(
              <button key={i} onClick={()=>setSz(i)} style={{padding:"7px 10px",borderRadius:10,background:sz===i?VERDE:CARD,color:sz===i?"#fff":TEXT2,border:sz===i?`1px solid ${VERDE2}`:`1px solid ${BORDER}`,fontSize:10,fontWeight:700,fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>{s.label}</button>
            ))}
          </div>
